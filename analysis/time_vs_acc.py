...
for index, exp_name in enumerate(
    ['adaptive_normal_nl_g', 'adaptive_normal_l_wg', 'adaptive_fast_nl_g', 'adaptive_fast_l_wg', ]):               try:
...
if exp_name == "adaptive":
    ...
    plt.plot(np.array(ms) / 1e6, marker=markers[index], markersize=5, markevery=10, linewidth=1)
...
y = ["FedPP", "Fast-FedPP1", "FedPP2", "FastFedPP2", ]
frameon = False, loc = "center right")
plt.xlabel("Round")
plt.ylabel("Number of Parameters ($\\times10^6$)")
plt.savefig(join(fig_path, "ablation_model_size{}".format("_cs" if client_sel else "")), dpi=300)
plt.close()
