...
class EarlyStopping:
    ...
    def __call__(self, val_loss,logger):
        ...
        if self.best_score is None:
            ...
            self.counter += 1
            logger.info(f'EarlyStopping counter: {self.counter} out of {self.patience}, best score is {self.best_score}, current score is {score}')
    ...
    def main(self, idx, list_sd, list_num_proc, lr, list_client_loss,list_client_acc, list_client_sum, start, list_loss, list_acc, list_est_time,
        ...
        if idx % self.config.EVAL_DISP_INTERVAL == 0:
            ...
            if self.display:
                ...
                print('the density is ' + str(client_density))
            ...
            if self.early_stoping(self.fed_avg_acc[-1], self.logger):
                ...
                if self.min_density == 1:
                    ...
                    if self.min_density + self.increse < 1:
                        ...
                        self.early_stoping.patience = self.early_stoping.patience*4
                        self.min_density = 1
                    ...
                    # self.fix_model()
                    list_state_dict, self.model_idx, sub_model_time, self.list_mask, self.list_sum_mask = self.control.sub_adjust_fast(
                        client_density=client_density, use_coff=self.use_coeff,
                        min_density=0.001, )

                    new_mask = self.list_mask[0]
                    if self.lr_warm:
                    ...
                    self.logger.info('the increase model is too small, continue  warm up '+str(idx))
                    print('the increase model is too small, continue  warm up')
...
class FedMapClient:
    ...
    def train_model(self, data_list, model_density,):
        ...
        if self.args.fair_degree == 0 :
            for i in range(self.config.NUM_LOCAL_UPDATES):
                 ...
                 list_grade, loss = self.optimizer_wrapper.step(inputs.to(self.device), labels.to(self.device))
                 num_proc_data += len(inputs)
            ...
            for i in range(self.config.NUM_LOCAL_UPDATES):
                 inputs, labels = data_list[i][0], data_list[i][1]
                 _, loss = self.optimizer_wrapper.step(inputs.to(self.device), labels.to(self.device))
                 num_proc_data += len(inputs)
    ...
    def main(self, idx, loss, logger, lr_mask_dict, warming_up=False):
        ...
        num_proc_data,loss = self.train_model(data_list,model_density)
    ...
    def main(self):
        ...
        while idx < self.max_round:
            ...
            for i in range(len(self.client_list)):
                ...
                client.load_state_dict(model_state_dict[i])
                if idx % self.config.EVAL_DISP_INTERVAL == 0:
                    client.optimizer_wrapper.lr_scheduler_step(self.server.list_client_acc[-1][i])
