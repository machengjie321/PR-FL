...
class FedMapServer(ABC):
    def __init__(self, config, args, model, seed, optimizer_class: Type, optimizer_params: dict,
                 ...
                 control_scheduler=None, resume=False, init_time_offset=0, device=None,data_set=None):
        self.config = config
    ...
    def save_display_data(self):
        ...
        mkdir_save(self.list_npmi, os.path.join(self.save_path, 'self.list_npmi'))
        mkdir_save(self.list_td, os.path.join(self.save_path, 'self.list_td'))
        mkdir_save(self.fed_avg_npmi, os.path.join(self.save_path, 'self.fed_avg_npmi'))
        mkdir_save(self.fed_avg_td, os.path.join(self.save_path, 'self.fed_avg_td'))
    ...
    def buff_fed_avg(self, list_num_proc, list_state_dict, idx, sgrd_to_upload):  # to complete the merge model ps: fedavg
        ...
        #     self.logger.info('use fedavg to merge client')
        # it`s for syn fed_avg
        coeff = [list_num_proc[i] * self.list_coeff[i] for i in range(len(self.list_client_sd))]
        list_state_dict = self.list_client_sd
        total_coeff = sum(coeff)
        self.old_model = copy.deepcopy(self.model.state_dict())
        with torch.no_grad():
    ...
    def main(self, idx, list_sd, list_num_proc, lr, list_client_loss,list_client_acc, list_client_sum, round_time, list_loss, list_acc, list_est_time,
        ...
        # list_sd for some function without buffer
        ...
        for i in range(len(self.client_density)):
            if self.train_number[i] % test_client_interval == 1:
                ...
                val_samples_processed, client_loss = client_model._evaluate(client_model, self.valid_data)
                output = client_model.predict(self.test_loader, client_model)
                accuracy = AccuracyScore(dataset)
                client_acc = accuracy.score(output)
        ...
        if interval_signal:
            val_samples_processed, loss = self.model._evaluate(self.model, self.valid_data)
            output = self.model.predict(self.test_loader, self.model)
            acc = accuracy.score(output)
            # Initialize metric
            ...
            if self.min_density < 0.99:
                ...
                val_samples_processed, avg_loss = fed_avg_model._evaluate(fed_avg_model, self.valid_data)
                output = fed_avg_model.predict(self.test_loader, fed_avg_model)
                avg_acc = accuracy.score(output)
                # Initialize metric
                # Retrieve metrics score
                avg_topic_diversity_score = topic_diversity.score(output)
                avg_npmi_score = npmi.score(output)

            else:
                avg_loss = loss
                avg_acc = acc
                avg_topic_diversity_score = topic_diversity_score
                avg_npmi_score= npmi_score
            self.list_loss.append(loss)
            self.list_acc.append(acc)
            self.list_td.append(topic_diversity_score)
            self.list_npmi.append(npmi_score)
            self.fed_avg_npmi.append(avg_npmi_score)
            self.fed_avg_td.append(avg_topic_diversity_score)
        ...
        # print(self.list_sparse_state_dict)
