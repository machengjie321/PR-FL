...
class EarlyStopping:
    ...
    def __init__(self, patience=10, verbose=False, delta=0):
        ...
        self.old_score = deque(maxlen=(patience))
...
class FedMapServer(ABC):
    ...
    def save_checkpoint(self, list_optimizer, list_lr_scheduler):
        ...
        checkpoint = {"self.sever_to_client_sum": self.sever_to_client_sum,
                      ...
                      'self.list_client_sum': self.list_client_sum,
                      'self.list_l2_client_sum': self.list_l2_client_sum,
...
client_l1_sum: tensor(10313.4512)   tensor(10388.3496)
client_l2_sum: tensor(15685111.)   tensor(15685114.)
client_l1_sum: tensor(10325.5303)   tensor(10400.5352)
client_l2_sum: tensor(15764251.)   tensor(15764254.)
client_l1_sum: tensor(10341.8018)   tensor(10416.9404)
client_l2_sum: tensor(15843689.)   tensor(15843692.)
client_l1_sum: tensor(10361.8711)   tensor(10437.2012)
client_l2_sum: tensor(15923383.)   tensor(15923387.)
client_l1_sum: tensor(10380.9629)   tensor(10456.4033)
client_l2_sum: tensor(16003270.)   tensor(16003275.)
...
"Because the weight of the pruned neuron is too small, I need to enlarge it to rejoin the training."
        ...
        with torch.no_grad():
            for layer in self.model.prunable_layers:
                sum = layer.weight.sum()
                num = torch.count_nonzero(layer.weight).item()
                mask = torch.abs(layer.weight) < sum / num
                layer.weight[mask] = sum / num
    ...
    def main(self, idx, list_sd, list_num_proc, lr, list_client_loss,list_client_acc, list_client_sum, start, list_loss, list_acc, list_est_time,
        ...
        self.list_client_density.append(copy.deepcopy(client_density))
        ...
        if self.round % self.config.EVAL_DISP_INTERVAL == 0:
            ...
            if self.early_stoping(self.fed_avg_acc[-1], self.logger):
                self.save_checkpoint(list_optimizer=list_optimizer, list_lr_scheduler=list_lr_scheduler)
            elif self.early_stoping.early_stop:
                ...
                if self.min_density == 1:
                    ...
                    mkdir_save(self.model_G, os.path.join(self.save_path, 'model_G.pt'))
                    self.early_stop = True
                elif self.warming_up == False:
                    if self.min_density + self.increse < 1:
                        self.min_density = self.min_density + self.increse
                    else:
                        self.early_stoping.patience = self.early_stoping.patience*2
                        self.min_density = 1
                    self.early_stoping.best_score = None
                    self.early_stoping.counter = 0
                    self.early_stoping.early_stop = False
                    for i in range(len(client_density)):
                        if client_density[i] < self.min_density:
                            client_density[i] = self.min_density
                    self.logger.info('client_density  '+str(client_density))
                    self.logger.info('the old density is too small, need increase the min model density '
                                     'to increase acc, current rond is '+str(idx))
                    print('the old density is too small, need increase the min model density '
                                     'to increase acc, current rond is '+str(idx)+'the new density is '+str(client_density))

                    self.old_mask = self.list_mask[0]
                    # self.fix_model()
                    list_state_dict, model_idx, sub_model_time, list_mask = self.control.sub_adjust_fast(
                        client_density=client_density, use_coff=self.use_coeff,
                        min_density=0.001, )
                    self.old_list_mask = list_mask
                    self.list_mask = list_mask
                    new_mask = list_mask[0]
                    if self.lr_warm:
                        for key, value in new_mask.items():
                            self.lr_mask_dict[key] = new_mask[key] ^ self.old_mask[key]
        ...
        for state_dict in list_state_dict:
            ...
            for key, param in state_dict.items():
                ...
                l2_sum += (param**2).sum()
            client_l1_sum.append(copy.copy(l1_sum.cpu()))
            client_l2_sum.append(copy.copy(l2_sum.cpu()))
        self.list_client_sum.append(client_l1_sum)
        self.list_l2_client_sum.append(client_l2_sum)
        old_model_sum = 0
        if self.min_density in self.config.L1_GAP.keys():
            for key , param in self.model.state_dict().items():
                if key in self.old_mask.keys():
                    old_model_sum += torch.abs(self.model.state_dict()[key]*self.old_mask[key]).sum()
                else:
                    old_model_sum += torch.abs(self.model.state_dict()[key]).sum()
            old_model_sum = old_model_sum.cpu()
            if (client_l1_sum[0] - old_model_sum)/client_l1_sum[0] > self.config.L1_GAP[self.min_density]:
                self.warming_up = False
                print('End of this warm up process at round ' + str(idx) + 'current client density is \n' + + str(client_density))
                self.logger.info('End of this warm up process at round ' + str(idx) + 'current client density is \n' + str(client_density))



        if self.round % self.config.EVAL_DISP_INTERVAL == 0:
            self.logger.info('client_l1_sum: '+str(old_model_sum if old_model_sum != 0 else "") +'   '+ str(client_l1_sum[0]) + '   ' + str(client_l1_sum[-1]))
            self.logger.info('client_l2_sum: ' + str(client_l2_sum[0]) + '   ' + str(client_l2_sum[-1]))
            print('client_l1_sum: '+str(client_l1_sum[0] - old_model_sum)+'  ' + str(client_l1_sum[0]) + '   ' + str(client_l1_sum[-1]))
            print('client_l2_sum: ' + str(client_l2_sum[0]) + '   ' + str(client_l2_sum[-1]))
...
class FedMapClient:
    ...
    def main(self, idx, loss, logger, lr_mask_dict, warming_up=False):
        if id(self.lr_mask_dict) == id(lr_mask_dict) or lr_mask_dict == {}:
            ...
            self.optimizer.lr_dict = self.lr_mask_dict
            self.optimizer.name_params = self.model.named_parameters()
        ...
        return state_dict_to_server, num_proc_data, lr, model_density
    ...
    def main(self):
        ...
        while idx < self.max_round:
            ...
            for client in self.client_list:
                client_time = timer()
                loss = client_loss[i]
                sd, npc, last_lr, ds = client.main(idx, loss, logger, self.server.lr_mask_dict,self.server.warming_up)
                i = i + 1
            ...
            list_state_dict, model_idx, sub_time,self.client_loss = self.server.main(idx, list_state_dict, list_num, last_lr,
                                                                                     list_client_loss, list_client_acc,
                                                                                    list_client_sum, server_time,
                                                                                    self.list_loss, self.list_acc, self.list_est_time,
                                                                                    self.list_model_size, is_adj_round, self.client_density,
                                                                                     list_optimizer, list_lr_scheduler)
