...
class ProdLDAServer(ABC):
    def __init__(self, args, config, model, device ,data_set=None):
        self.config = config
        self.device = device
        ...
        # Initialize metric
        self.npmi = Coherence(texts=self.data_set.get_corpus(), topk=10, measure='c_npmi')
        self.topic_diversity = TopicDiversity(topk=10)
        self.accuracy = AccuracyScore(self.data_set)
    ...
    def main(self, idx, list_sd, list_num_proc, lr, list_accumulated_sgrad, start, list_topic_diversity,list_npmi,list_accuracy, list_recall, list_precision, list_f1, list_loss, list_est_time,
        ...
        if idx % self.config.EVAL_DISP_INTERVAL == 0:
            ...
            accuracy_score = self.accuracy.score(output)
            print("accuracy_score={}".format(accuracy_score))
            list_accuracy.append(accuracy_score)
            # Initialize metric

            # Retrieve metrics score
            topic_diversity_score = self.topic_diversity.score(output)
            print("Topic diversity: " + str(topic_diversity_score))
            npmi_score = self.npmi.score(output)
...
class ProdLDAClient:
    def __init__(self, model, config, use_adaptive, args,device):
        self.config = config
        self.device = device
    ...
    def main(self, is_adj_round):
        ...
        for _ in range(self.config.NUM_LOCAL_UPDATES):
            ...
            inputs = self.train_loader.get_next_batch()
...
class ProdLDAFL(ABC):
    def __init__(self, args, config, server, client_list):
        ...
        self.use_adaptive = args.use_adaptive
        self.tgt_d, self.max_d = args.target_density, 1
            ...
            if self.use_adaptive:
                ...
                if self.prune_speed == 'nl':
                    if self.prune_type == 'normal':
                        ...
                        density_limit =  self.tgt_d + (1-self.tgt_d)*0.95**(idx*100.0/self.max_round*4)
