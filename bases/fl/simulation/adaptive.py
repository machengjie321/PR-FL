...
def parse_args():
    ...
    parser.add_argument('-e', '--exp-name',
                        ...
                        type=str,
                        required=True)
    parser.add_argument('-pt', '--prune_type',
                        help="Fast or Normal prune type",
                        action='store',
                        dest='prune_type',
                        type=str,
                        required=True)

    parser.add_argument('-ps', '--prune_speed',
                        help="nl or l prune_speed",
                        action='store',
                        dest='prune_speed',
                        type=str,
                        required=True)

    parser.add_argument('-pm', '--prune_merit',
                        help="w or wg prune merit",
                        action='store',
                        dest='prune_merit',
                        type=str,
                        required=True)
...
class AdaptiveServer(ABC):
    def __init__(self, args, config, model, save_interval=50):
        ...
        self.prune_merit = args.prune_merit
    ...
    def main(self, idx, list_sd, list_num_proc, lr, list_accumulated_sgrad, start, list_loss, list_acc, list_est_time,
        ...
        if self.use_adaptive and is_adj_round:
            ...
            for d in list_accumulated_sgrad:
                for k, sg in d.items():
                    if self.prune_merit == 'wg':
                        old_weight = old_model[k]
                        self.control.accumulate_wg(k,old_weight)
                    elif self.prune_merit == 'g':
                        self.control.accumulate_w(k)
            ...
            print("Total alg time = {}. current density = {}.".format(timer() - alg_start, density_limit))
            print("Num params:")
...
class AdaptiveClient:
    ...
    def main(self, is_adj_round):
        ...
        self.optimizer = SGD(self.model.parameters(), lr=self.config.INIT_LR)
        self.optimizer_wrapper = OptimizerWrapper(self.model, self.optimizer)
        ...
        for i in range(self.config.NUM_LOCAL_UPDATES):
            ...
            list_grad = self.optimizer_wrapper.step(inputs, labels)

            num_proc_data += len(inputs)

            for (key, param), g in zip(self.model.named_parameters(), list_grad):
                # assert param.size() == g[0].size()  # only simulation
...
class AdaptiveFL(ABC):
    def __init__(self, args, config, server, client_list):
        ...
        self.prune_speed = args.prune_speed
    ...
    def main(self):
        ...
        start = timer()
        m_rouond = self.max_round
        if self.prune_type == 'fast':
            m_rouond = m_rouond*2
        for idx in range(m_rouond):
            ...
            if self.tgt_d is not None:
                ...
                if self.prune_speed == 'nl':
                    if self.prune_type == 'normal':
                        density_limit =  self.tgt_d + (1-self.tgt_d)*0.95**(idx*100.0/self.max_round*1.1)
                    elif self.prune_type == 'fast':
                        density_limit =  self.tgt_d + (1-self.tgt_d)*0.95**(idx*100.0/self.max_round*1.1)
                elif self.prune_speed == 'l':
                    if self.prune_type == 'normal':
