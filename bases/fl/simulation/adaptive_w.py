...
class AdaptiveFL(ABC):
    ...
    def slower_decreasing_function(self, n, total_steps=50.0, start=1, end=0.1):
        """
        A function that decreases from 1 to 0.1 over 50 steps. The decrease becomes slower as n approaches total_steps.
        This uses an inverse exponential approach to slow down the decrease.
        """
        # Normalize the current step
        normalized_step = n / total_steps
        # Calculate the decrease using an inverse exponential approach
        value = 1-(start - end) * (1 - np.exp(-5 * normalized_step))
        return value
        ...
        for idx in range(self.max_round):
            ...
            if self.tgt_d is not None:
                ...
                if self.prune_type == 'normal':
                    density_limit =  self.tgt_d + (1-self.tgt_d)*0.95**(idx*100/self.max_round*1.1)
