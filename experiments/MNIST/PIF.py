...
class args:
    def __init__(self):
        ...
        self.experiment_name = self.experiment_name + '_' + str(self.density) + str(self.min_density) + '_' + (
            '' if self.chronous == 'syn' else '_' + self.chronous + '_') + str('_recover' if self.recover else '') \
                               + '_' + str(self.stal.lower()) + '_' + self.merge + '_' + self.sample + '_' + str(
            '' if self.stal_a == 0.6 else self.stal_a) + str(
            '' if self.fair == 'un_fair' else '_' + self.fair) + '_' + str(
            '' if self.fair_degree == 0 else self.fair_degree) + '_' + str(
            self.interval) + '_' + str('' if self.increase == 0.2 else self.increase) + '_' + str(
            '' if self.weight_decay == 0 else
            self.weight_decay) + '_' + str('' if self.accumulate == 'g' else self.accumulate) + str(
            '_clr' if self.control_lr else '') + '_' + str('' if
                                                           self.wdn == 10 else self.wdn) + str(
            '_ft' if self.ft == 'n' else '') + str('_lr_scheduler' if self.lr_scheduler else '') + str(
...
if __name__ == "__main__":
    ...
    server = FEMNISTFedMapServer(config, args, Conv2Mnist(), seed, SGD, {"lr": config.INIT_LR}, use_adaptive, device)
